...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function setupWidget(container, elements, config, data) {
        ...
        $(elements.dataContainer).off('change').on('change', 'input, select', function() {
...
function createRowContent(fieldsToUse, accountLabel, elements) {
    ...
    if (accountLabel === "Entity") {
        fieldsToUse.fields = fieldsToUse.fields.filter(field =>
            !["beneficiaryLabel", "beneficiaryYN", "beneficiaryName", "beneficiaryPhoneNumber"].includes(field.name)
        );
    }
    ...
    if (isVisible) {
        ...
        $rows.each(function(index) {
            ...
            if (registrationType || otherRegistration) {
                ...
                if (Object.keys(rowData).length > 2) { // More than just rowIndex and accountType
